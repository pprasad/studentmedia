...
exports.loginAuth=(loginType,parameters)=>{
	...
	if(model.LOGIN_TYPE_USERNAME===loginType){
	   query=db.prepareQuery("SELECT PASSWORD FROM STUD_REG_USER WHERE USER_NAME=?",parameters);
	}else{
	   query=db.prepareQuery("SELECT PASSWORD FROM STUD_REG_USER WHERE EMAIL_ID=?",parameters);	
	}
